...
from fixposcdm import fit_linear_map, anyclose
...
def get_corresp(ref_crv, ref_par, curves):
    ...
    for crv in curves:
        ...
        cor_par.append(peaks[np.argmin(np.abs(peaks - ref_par))])
        cor_poi.append(crv[:, cor_par[-1]])

    return cor_poi, cor_par


def get_features(curve, param, poi):
...
class FixKrvCDM:
    """Find the subspace where the curve hase the same curvature at the PoIs."""

    def __init__(self):
        # Initial parameters.
        self.disc_prop = (7, 3)
        self.cont_prop = (13., 2.8, 14., 2.)
        self.pts_per_dim = 5
        self.nbhood_size = .1
...
#==============================================================================
    ...
    def project_cont_prop_vect(self):
        ...
        def objective(p):
            ...
            feat = get_features(crv, par[0], poi[0])
            d_feat = ref_feat - feat
#            d_feat = (ref_feat - feat) / ref_feat
            d_init = p - init
    ...
    def get_invar_scores(self, samples):
        ...
        pois, pars = get_corresp(curves[-1], self.ref_par, curves[:-1])
        pois.append(self.ref_poi)
        pars.append(self.ref_par)
        feats = np.array([get_features(crv, par, poi)
                          for crv, par, poi in zip(curves, pars, pois)])
        dists = get_dist(feats[-1], feats[:-1].T)
        return np.exp(-dists)
#        diff = (feats[:-1] - feats[-1]) / feats[-1]
#        diff = np.linalg.norm(diff.T, axis=0)**2
#        return np.exp(-diff)
#        return np.exp(-np.diag(np.dot(diff, diff.T)))
