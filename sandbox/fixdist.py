...
def get_corresp_param(ref_crv, ref_par, curves):
    ...
    Here several reference param values are allowed. Returns an N_curves-list
    of N_ref_par-lists.
    """
    # For each curve extract candidate PoIs.
    poiss = []
    ol = 3 # overlap: to detect peaks at the interval's ends.
    for crv in curves:
        krv = compute_curvature(crv)
        n = len(krv)
        krv = np.hstack([krv[-ol:-1], krv, krv[1:ol]])
        peaks = sig.argrelmax(krv)[0] - (ol-1)
        peaks = peaks[peaks < n]
        poiss.append(peaks)
    # For each ref_par find in each curve the corresponding poi_par.
    corresp = []
    for par in ref_par:
        # Here we compare each id to the ref id but nearest neighbor might
        # work better.
        par_corresp = [pois[np.argmin(get_dist(par, pois))] for pois in poiss]
        corresp.append(par_corresp)
    return list(zip(*corresp)) # Trick to transpose the list
...
#        self.ref_par = (11, 117)
        self.ref_par = (53, 267)
        self.ref_par = get_corresp_param(
            self.ref_crv, self.ref_par, [self.ref_crv])[0]
        print(self.ref_par)
        ...
        self.inv_crv = fit_curve(self.samples[:, ids], w=self.scores[ids], d=3)
        # Redefine bounds.
...
#        self.make_gif()
    ...
    def make_gif(self):
        ...
        for i, val in enumerate(e2):
            ...
            plt.savefig("fixdist_explore_{0:02d}".format(i))
...
#        bounds = self.mecha.get_prop_bounds(2)
    ...
    def update(self, id_, value):
        ...
        self.new_par = get_corresp_param(self.ref_crv, self.ref_par,
                                         [self.new_crv])
        self.redraw()
    ...
    def redraw(self):
        ...
        new_poi = self.new_crv[:, self.new_par].T
    ...
    def get_optimal_path(self):
        ...
        ref_ft = get_features([self.ref_crv], [self.ref_par])[0]
        ...
        for val in e2:
            ...
            def obj_func(x):
                ...
                par = get_corresp_param(self.ref_crv, self.ref_par, [crv])[0]
                ft = get_features([crv], [par])[0]
