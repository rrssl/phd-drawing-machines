...
def fit_linear_map(s, w=None, ndim=4):
    """Fit a linear model to the (optionally) weighted samples."""
    pca = WPCA(n_components=ndim)
    if w is not None:
        w = np.tile(w, (s.shape[0], 1))
    pca.fit(s.T, weights=w.T)
    print(pca.explained_variance_ratio_)

    def phi(p):
        return pca.inverse_transform(np.asarray(p).reshape(ndim, -1).T).T
    def phi_inv(p):
        return pca.transform(np.asarray(p).reshape(s.shape[0], -1).T).T
...
#        self.nb = 2**5
        ...
        self.samples = np.array(list(
            self.sample_local_props(self.grid_size, frac=.1)
            )).T
        self.scores = self.get_invar_scores()
        # Filter out low scores and find linear map of invariant space.
        ids = get_highest_quantile(self.scores, q=40)
        self.phi, self.phi_inv = fit_linear_map(self.samples[:, ids],
                                                self.scores[ids])
        self.new_cont_prop = self.phi_inv(self.cont_prop)
#        self.inv_crv = fit_curve(
#            np.hstack([self.samples[:, ids], self.ref_par.reshape(-1, 1)]),
#            w=np.hstack([self.scores[ids], 1.]))
        # Redefine bounds.
        self.invar_space_bnds = [(-1., 1.)]*len(self.new_cont_prop)
...
#        self.make_gif()
    ...
    def make_gif(self):
        ...
        rng = np.linspace(*self.inv_crv_bnds, num=15)
...
#        bounds = self.mecha.get_prop_bounds(2)
    ...
    def update(self, id_, value):
        ...
        cont_prop = self.phi(self.new_cont_prop)
        ...
        for i in range(len(cont_prop)):
            bounds = self.mecha.get_prop_bounds(i+2)
            self.ref_control.set_bounds(i, bounds)
